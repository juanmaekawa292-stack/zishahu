...
export async function GET(request: NextRequest) {
  ...
  if (action === "track") {
    ...
    const now = new Date();
    const ago = (d) => { const t = new Date(now); t.setDate(t.getDate() - d); return t.toISOString().replace("T", " ").substring(0, 19); };
...
'''
os.makedirs(r'F:codex-yunxingzishahusrcappapishipping', exist_ok=True)
with open(r'F:codex-yunxingzishahusrcappapishippingoute.ts', 'w', encoding='utf-8') as f:
    f.write(route)
print('Written', len(route))
