...
const priorityI18nKey: Record<string, string> = {
...
const subjectI18nKey: Record<string, string> = {
  order: "ticketCategoryOrder",
  shipping: "ticketCategoryShipping",
  return: "ticketCategoryReturn",
  product: "ticketCategoryProduct",
  other: "ticketCategoryOther",
};
        ...
        {Object.entries(statusCounts).map(([key, count]) => (
          ...
          >
            ...
            <p className="text-[10px] text-muted-foreground">
              {key === "all" ? t("adminFilterAll") : t(statusI18nKey[key] || key)}
            </p>
          </button>
        ))}
      </div>
      ...
      <div className="rounded-lg border border-border bg-card">
        {filtered.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">{t("adminNoTickets")}</p>
          </div>
        ) : (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span className={cn(
                      ...
                      {ticket.priority === "urgent" ? "!" : ticket.id.slice(-3)}
                    </span>
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">
                        {ticket.customer} - {t(subjectI18nKey[ticket.subject] || ticket.subject)}
                      </p>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={cn(
                      ...
                      {t(priorityI18nKey[ticket.priority])}
                    </span>
                    <span className={cn(
                      "rounded-full px-2 py-0.5 text-[10px] font-medium",
                      statusColors[ticket.status]
                    )}>
                      {t(statusI18nKey[ticket.status])}
                    </span>
